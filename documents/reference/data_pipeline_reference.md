...
- Materialise global quarterly marts plus filtered marts for SVR-RM (overall + top customers), Synology C2 adoption cohorts, and commercial activation storylines; keep cohort filters in shared configs so forecasting and BI stay consistent.
...
| Inputs | Aggregated quarterly sales history CSVs or dbt outputs scoped to SVR-RM T1/T2/T3 (overall + top customers) and Synology C2 exploration cohorts |
| Outputs | Timestamped next-quarter SVR-RM and Synology C2 exploratory forecast CSVs + metadata JSON under `data/processed/forecasts/`; activation insights persisted alongside forecasts |
...
### Next Steps
- Expand feature set by querying dbt marts (e.g., promotions, inventory levels) tied to SVR-RM and Synology C2 performance.
- Track accuracy metrics (MAPE, sMAPE) for the next-quarter horizon per cohort and store in metadata files; alert if MAPE exceeds the business-defined tolerance.
