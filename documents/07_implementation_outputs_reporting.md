...
- Host interactive dashboards in Tableau Cloud with governed access controls
...
2. Configure Tableau Cloud projects/dashboards, ensuring live or extract connections to Supabase marts
3. Automate report generation post-forecast run, embedding charts (matplotlib/plotly) and tables
4. Trigger Tableau extract refreshes through Airflow or n8n and store static outputs with semantic filenames (date + model version), updating the changelog
5. Provide stakeholder distribution plan (email, shared drive, Tableau subscriptions)
...
- Tableau Cloud for dashboard delivery and subscriptions; Supabase as the data source
- matplotlib/plotly/seaborn for visualization; jinja2 or nbconvert for templated reports
- Tableau REST API or tabcmd CLI invoked via Airflow/n8n; Markdown/HTML renderers and CLI wrappers

## Risks & Mitigations
- Report drift: maintain contract tests and review loops with stakeholders
- Sensitive data exposure: apply masking rules, Tableau row-level security, and redact PII before distribution
